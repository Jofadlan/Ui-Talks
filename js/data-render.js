...
const SPREADSHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vRmCoT-NllVz0tti7Xv0lqvg47mYC0NOLbnGASvlzLnm7kCpqinpoRtR8H0xrtJyON7mWag21DPkv7b/pub?gid=0&single=true&output=csv";
const EVENTS_CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vRmCoT-NllVz0tti7Xv0lqvg47mYC0NOLbnGASvlzLnm7kCpqinpoRtR8H0xrtJyON7mWag21DPkv7b/pub?gid=163766779&single=true&output=csv";
const GALLERY_CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vRmCoT-NllVz0tti7Xv0lqvg47mYC0NOLbnGASvlzLnm7kCpqinpoRtR8H0xrtJyON7mWag21DPkv7b/pub?gid=1924427979&single=true&output=csv";
  ...
  try {
    ...
    const cards = rows
      ...
      .map((cols) => {
        ...
        const fotoUrl =
          cols[2] && cols[2].trim() !== "" ? cols[2].trim() : FALLBACK_IMAGE;
        return `
  ...
  try {
    ...
    const cards = rows
      ...
      .map((cols) => {
        ...
        const fotoUrl =
          cols[3] && cols[3].trim() !== "" ? cols[3].trim() : FALLBACK_IMAGE;
        const accentRaw = (cols[4] || "").trim().toLowerCase();
        const accentClass = ["blue", "orange", "green"].includes(accentRaw)
          ? `accent-${accentRaw}`
          : "";
...
async function loadGoogleSheetGallery() {
  ...
  try {
    ...
    topRow.innerHTML =
      ...
      `<div class="loading-text">Belum ada foto.</div>`;
